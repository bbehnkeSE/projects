...
void AST::mainHeader(const std::vector<std::string>& profileName) {
  ...
  for(auto it = child.begin(); it != child.end(); ++it) {
    if((*it) -> tag == "function") {
      ...
      std::string tmp;
      size_t size;
      for(auto vit = profileName.begin(); vit != profileName.end(); ++vit) {
	size = vit -> find_last_of('_');
	tmp = vit -> substr(0, size);	
	child.insert(it, new AST(token, "profile " + *vit + "(\"" + tmp + ".cpp\");\n\n")); 
      }
      break;
    }
  }
